...
public class Stream_05_Test {
    ...
    private long iterateSum(long n) {
        return Stream.iterate(0L, i -> i+1).limit(n).mapToLong(i -> i).sum();
    }
    ...
    private long parallelIterateSum(long n) {
      ...
      return Stream.iterate(0L, i -> i+1).limit(n).parallel().mapToLong(i -> i).sum();
    }
    ...
    public void monitor_imperativeSum_vs_iterateSum_vs_parallelIterateSum() {
      ...
      long n = 10000000;
        ...
        Logger.getAnonymousLogger().info("imperativeSum : " +  monitor(this::imperativeSum, n) + " ms");
        Logger.getAnonymousLogger().info("iterateSum :" + monitor(this::iterateSum, n) + " ms");
        Logger.getAnonymousLogger().info("parallelIterateSum :" + monitor(this::parallelIterateSum, n) + " ms");
      Logger.getAnonymousLogger().info("rangeSum :" + monitor(this::rangeSum, n) + " ms");
      Logger.getAnonymousLogger().info("rangeParallelSum :" + monitor(this::rangeParallelSum, n) + " ms");
    }

    // Quel résultat obtenez-vous ?
    // Sur ma machine, le gagnant est ... imperativeSum !
    // INFO: imperativeSum => 2 ms
    // INFO: iterateSum => 58 ms
